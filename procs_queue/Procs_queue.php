	...
	*/
	...
	public function add_worker(string $user, string $host, string $ssh_public_key, string $ssh_private_key, string $proc_path, string $tmp_path): self{
		try{
			...
			$ssh = new Worker_init($user, $host, false, $ssh_public_key, $ssh_private_key);
	...
	private function read_proc_streams(): void{
		...
		foreach($this->procs as $p => $proc){
			...
			if(!$proc['cmd']->is_running()){
				$exitcode 	= $this->parse_exitcode(file_exists($proc['exitcode']) ? trim(file_get_contents($proc['exitcode'])) : '');
